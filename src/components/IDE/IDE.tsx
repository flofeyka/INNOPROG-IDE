...
import StartFormModal from "../StartFormModal/StartFormModal";
...
interface WebSocketData {
	...
	>;
	codeEdits: Map<string, CodeEditData>;
	myUserColor: string;
...
const IDE: React.FC<IDEProps> = React.memo(({ webSocketData }) => {
	...
	// Мемоизируем setCode для предотвращения пересоздания функции
	...
	useEffect(() => {
		...
		const currentMember = webSocketData.roomMembers?.find(
			(member) => member.telegramId === myTelegramId
		);
	...
	useEffect(() => {
		const handleRoomStateLoaded = (event: CustomEvent) => {
			...
			if (lastCode && lastCode.trim()) {
				// ⚠️ ВАЖНО: код из комнаты - это ТОЛЬКО редактируемая часть
				// Нередактируемые части (codeBefore/codeAfter) всегда должны браться из задачи
				// Это предотвращает конфликт между кодом задачи и кодом комнаты

				// Проверяем, есть ли нередактируемые части в загруженном коде
				// Если есть, то это старая версия - извлекаем только редактируемую часть
	...
	const memoizedWebSocketData = useMemo(() => {
		...
		webSocketData?.sendCodeEdit,
		webSocketData?.codeEdits,
